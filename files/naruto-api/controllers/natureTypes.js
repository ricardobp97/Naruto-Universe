...
module.exports.getAllNatureTypes = async () => {
    let query = `SELECT ?id ?name WHERE {
        ?id rdf:type :NatureType .
        ?id :name ?name .
    }`;
...
module.exports.getNatureType = async (idNatureType) => {
    let query = `SELECT ?name ?description WHERE {
        :${idNatureType} :name ?name .
        :${idNatureType} :description ?description .
    }`;
    let encoded = encodeURIComponent(prefixes + query);
    try {
        let response = await axios.get(getLink + encoded);
